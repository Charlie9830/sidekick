# AI Rules for Flutter with Shadcn UI`
## Core Philosophy
- Follow Shadcn's guiding principle: "You own the components." Keep layouts flexible, modular, and un-overengineered.
- Prioritize composition over heavy inheritance, deep widget trees, or ultra-specific custom variants.
- Design systematically, ensuring full cross-platform compatibility (iOS, Android, Web, Desktop).

## Framework & Dependencies
- Use the `shadcn_flutter` / `shadcn_ui` package ecosystem as a standalone UI design system.
- When writing layout files, do not mix generic Material/Cupertino styles unless explicitly requested.
- Use `lucide_icons` (bundled) and `flutter_animate` for UI transitions.

## Code Style & Constraints
- Always use `const` constructors where possible to optimize Flutter rendering performance.
- Prefer `gap` parameters inside layout containers rather than nesting items inside `SizedBox` spacing widgets.
- Enforce strict semantic tokens (e.g., `theme.colorScheme.primary`) instead of raw hex values or hardcoded Material Colors.
- Maintain clean layout boundaries: ensure scrollable views use explicit dimension bounds or constraints so they never crash the UI.

## UI & Form Patterns
- Build forms using compound layout variants provided by the package (such as explicit Form fields or built-in InputOTP/DatePicker fields) rather than raw generic text fields.
- Always verify accessibility attributes: dialogs, drawers, and sheets must be assigned explicit text titles even if hidden visually.
- Use clean, minimal layout cards and glassmorphism styles natively supported by the custom theme configuration.
