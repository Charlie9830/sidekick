...
class ProjectFileModel {
  ...
  ProjectFileModel copyWith({
    ...
    List<FixtureGeometryModel>? fixtureGeometries,
  }) {
  ...
  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      ...
      'fixtureGeometries': fixtureGeometries.map((x) => x.toMap()).toList(),
    };
  }
  ...
  factory ProjectFileModel.fromMap(Map<String, dynamic> map) {
    return ProjectFileModel(
      ...
      defaultPowerMulti: CableType.values.byName(
        map['defaultPowerMulti'] ?? CableType.socapex.name,
      ),
      ...
      fixtureTypes: List<FixtureTypeModel>.from(
        (map['fixtureTypes'] ?? []).map<FixtureTypeModel>(
      ...
      powerFeeds: List<PowerFeedModel>.from(
        (map['powerFeeds'] ?? []).map<PowerFeedModel>(
          (x) => PowerFeedModel.fromMap(x as Map<String, dynamic>),
        ),
      ),
      powerRacks: List<PowerRackModel>.from(
        (map['powerRacks'] ?? []).map<PowerRackModel>(
      ...
      dataRacks: List<DataRackModel>.from(
        (map['dataRacks'] ?? []).map<DataRackModel>(
      ...
      fixtureTypePools: List<FixtureTypePoolModel>.from(
        (map['fixtureTypePools'] ?? []).map<FixtureTypePoolModel>(
          (x) => FixtureTypePoolModel.fromMap(x as Map<String, dynamic>),
        ),
      ),
      trusses: List<TrussModel>.from(
        (map['trusses'] ?? []).map<TrussModel>(
          (x) => TrussModel.fromMap(x as Map<String, dynamic>),
        ),
      ),
      fixtureGeometries: List<FixtureGeometryModel>.from(
        (map['fixtureGeometries'] ?? []).map<FixtureGeometryModel>(
          (x) => FixtureGeometryModel.fromMap(x as Map<String, dynamic>),
        ),
      ),
    );
  }
  ...
  FixtureState toFixtureState() {
    return FixtureState(
      ...
      fixtureGeometries: fixtureGeometries.toModelMap(),
    );
  }
}
