...
/// [cables] Represents cables which have been modified by this operation, including new Parent Sneak Cables.
...
CombineIntoMultiResult combineDmxIntoSneak({
      ...
      .map((entry) {
        ...
        return childSlices.mapIndexed((index, slice) {
          ...
          final associatedOutlets =
              slice.map((cable) => outlets[cable.outletId]).nonNulls.toList();
          ...
          return (sneak, multiOutlet, withSparesFilled, targetLocationModel);
        });
      })
      .flattened
      .toList();
  ...
  final locations =
      newSneaksWithOutletsAndChildren.map((item) => item.$4).toList();
...
CombineIntoMultiResult combineHoistsIntoMulti({
  ...
  // Map through the Cables by Loom collection, returning a conjunction of the Motor Multi, it's new outlet and the children to be assigned to it.
  final List<
      (
        CableModel sneak,
        HoistMultiModel outlet,
        List<CableModel> children,
        LocationModel targetLocation
      ...
      .map((entry) {
        ...
        return childSlices.mapIndexed((index, slice) {
          ...
          return (multi, multiOutlet, withSparesFilled, targetLocationModel);
        });
      })
      .flattened
      .toList();
  ...
  final locations =
      newMultisWithOutletsAndChildren.map((item) => item.$4).toList();
