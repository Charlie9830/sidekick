...
import 'package:sidekick/screens/file/import_module/read_raw_trusses.dart';
...
class ImportManager extends StatefulWidget {
  ...
  const ImportManager({super.key, required this.vm});
...
class _ImportManagerState extends State<ImportManager> {
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              SimpleTooltip(
                message: canProgress == false ? _getProgressHaltedText() : null,
                child: PrimaryButton(
                  density: ButtonDensity.iconComfortable,
  ...
  bool _canProgress() {
    ...
    return switch (currentStep) {
      ImportManagerStep.fileSelect =>
        _isFixtureDatabasePathValid &&
    ...
    return allFixtureIds.map((uid) {
      ...
      final incomingFixtureVm = incomingFixture == null
          ...
          : FixtureViewModel(
              ...
              location:
                  incomingLocationLookup[incomingFixture
                              .mvrLocationId
                              .isNotEmpty
  ...
  void _loadMergeDataStep() async {
    ...
    final incomingLocationsByName = Map<String, RawLocationModel>.fromEntries(
      _incomingLocations.map((incoming) => MapEntry(incoming.name, incoming)),
    );

    final locationMappings = Map<String, String>.fromEntries(
      existingLocationsByName.entries
          .map(
  ...
  void _handleFinish() {
    final fixtureTypesByShortName = _getFixtureDatabaseEntriesByShortName(
      _fixtureTypes,
    );

    List<FixtureModel> fixtures = _incomingFixtures.mapIndexed((
      index,
      incomingFixture,
    ) {
    ...
    final locations = [
      ...
      ..._incomingLocations.map((incomingLocation) {
        ...
        if (existingMvrLocation != null) {
          ...
          return existingMvrLocation.copyWith(name: incomingLocation.name);
        }

        final existingMappedLocation = widget
            .vm
        ...
        if (existingMappedLocation != null) {
          ...
          return existingMappedLocation.copyWith(name: incomingLocation.name);
        }
        ...
        return LocationModel(
          ...
          delimiter: LocationModel.getDefaultDelimiterValue(
      ...
      ...widget.vm.existingLocations.values.where(
        (location) => location.isHybrid,
      ),

      // Marry with existing Rigging Only Locations.
      ...widget.vm.existingLocations.values.where(
        (location) => location.isRiggingOnlyLocation,
      ),
    ];
  ...
  List<FixtureMappingViewModel> _selectFixtureMappingViewModels(
    ...
    final databaseEntriesByShortName = _getFixtureDatabaseEntriesByShortName(
      _fixtureTypes,
    );
    ...
    return fixtureMappings.entries.map((entry) {
      ...
      return FixtureMappingViewModel(
        mapping: mapping,
        existsInDatabase: databaseEntriesByShortName.containsKey(
          mapping.mappedFixtureType,
        ),
      );
    }).toList();
  }
  ...
  void _loadFixtureMappingStep() async {
    ...
    final fixtureMatchers = await _loadFixtureMapping(
      widget.vm.fixtureMappingFilePath,
    );
    ...
    final fixtureDatabaseResult = await _loadFixtureDatabase(
      widget.vm.fixtureDatabaseFilePath,
    );
